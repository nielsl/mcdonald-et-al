...
# display structure and state is saved, and an anatomy database to pull
# from (TODO: de-couple the database). The database accessors are in 
# Anatomy::DB and the tree memory manipulation routines are in 
# Common::DAG::Nodes. 
...
@EXPORT_OK = qw (
                 ...
                 &display_menu_row
                 &main
                 &display_mouse_report
                 &display_plant_report
                 &display_rows
                 &display_row
                 &display_stats_cell
                 &parents_menu
...
sub add_checkbox_row
{
    ...
    foreach $info ( &Anatomy::Menus::control_info() )
    {
...
sub add_col_styles
{
    ...
    $ramp_keys =
    {
        $db_prefix."_terms_usum" => [ "#cccccc", "#ffffff" ],
        $db_prefix."_terms_tsum" => [ "#cccccc", "#ffffff" ],
    };

    @node_ids = keys %{ &Common::DAG::Nodes::get_ids_subtree_unique( $nodes, $state->{"ana_root_id"} ) };
...
sub add_checkbox_column
{
    ...
    $options = &Anatomy::Menus::control_info( $db_prefix );    
...
sub add_data_column
{
    ...
    $option = &Anatomy::Menus::data_info( $db_prefix )->[ $optndx ];
    ...
    elsif ( $key eq "ana_icd9" )
    {
        $xrefs = &Anatomy::DB::get_xrefs( $dbh, "ICD9", $node_ids );
...
sub add_statistics_column
{
    ...
    $option = &Anatomy::Menus::statistics_info( $db_prefix )->[ $optndx ];
    ...
    $stats = &Anatomy::DB::get_statistics( $dbh, $key, $node_ids );
...
sub create_columns
{
    ...
    for ( $col_index = 0; $col_index <= $#{ $columns }; $col_index++ )
    {
        ...
        {
            if ( $key eq "checked" ) {
                $display = &Anatomy::Viewer::add_checkbox_column( $dbh, $display, "checked", $root_id );
            } elsif ( $key eq "unchecked" ) {
                $display = &Anatomy::Viewer::add_checkbox_column( $dbh, $display, undef, $root_id );
            }
        }
        elsif ( $type eq "statistics" )
        {
            $display = &Anatomy::Viewer::add_statistics_column( $dbh, $display, $opt_index,
                                                                $col_index, $root_id, $status );
        }
        elsif ( $type eq "data" )
        {
            $display = &Anatomy::Viewer::add_data_column( $dbh, $display, $opt_index, 
                                                          $col_index, $root_id );
        }
    }
...
sub display_action_buttons
{
    ...
    if ( $state->{"ana_save_rows_button"} )
    {
        $xhtml .= qq (<td valign="middle">)
                   . &Anatomy::Widgets::save_selection_button( $db_prefix, $sid )
                   . "</td>";
        $flag = 1;
    }

    if ( $state->{"ana_delete_rows_button"} and defined $state->{"ana_selections_menu"} )
    {
         $index = $state->{"ana_selections_menu"};
        
         $xhtml .= qq (<td valign="middle">)
                   . &Anatomy::Widgets::delete_selection_button( $db_prefix, $sid, $index ) 
                    . "</td>";
         $flag = 1;
    }

    if ( $state->{"ana_delete_cols_button"} )
    {
        $xhtml .= qq (<td valign="middle">)
                   . &Anatomy::Widgets::delete_columns_button( $sid ) 
                   . "</td>";
...
sub display_checkbox_cell
{
    ...
    if ( $value eq "1" ) {
        $xhtml .= qq (<td>&nbsp;<input type="checkbox" name="ana_row_ids" value="$node_id" class="checkbox" checked />&nbsp;</td>);
    } else {
        $xhtml .= qq (<td>&nbsp;<input type="checkbox" name="ana_row_ids" value="$node_id" class="checkbox" />&nbsp;</td>);
    }
...
sub display_checkbox_row
{
    ...
    if ( grep { exists $_->{"checked"} } @{ $display->{"columns"} } )
    {
        ...
        $icon_xhtml = &Anatomy::Widgets::cancel_icon();
        ...
        for ( $i = 0; $i <= $#{ $columns }; $i++ )
        {
            ...
            {
                if ( $value ) {
                    $xhtml .= qq (<td class="$style"><input name="ana_col_ids" type="checkbox" value="$i" checked></td>);
                } else {
                    $xhtml .= qq (<td class="$style"><input name="ana_col_ids" type="checkbox" value="$i"></td>);
                }
            }
        }
...
sub display_col_headers
{
    ...
    $root_node = $nodes->{ $state->{"ana_root_id"} };
    ...
    for ( $i = 0; $i <= $#{ $columns }; $i++ )
    {
        ...
        if ( $type eq "checkbox" )
        {
            ...
            $args = qq (WIDTH,180,CENTER,BELOW,OFFSETY,20,CAPTION,'$menu',FGCOLOR,'#ffffcc',BGCOLOR,'#006666',BORDER,3);
            $style = "ana_col_title";
        }
        else
        {
    ...
    {
        $xhtml .= "<td>". &Anatomy::Viewer::parents_menu( $dbh, $nodes, $state ) ."</td>";
    }
...
sub display_id_cell
{
    ...
    if ( defined $value )
    {
        $url = qq ($Common::Config::cgi_url/index.cgi?session_id=$sid;page=$db_prefix;request=ana_report;ana_report_id=$id);
        $text = $value;
...
sub display_menu_row
{
    ...
    if ( $state->{"ana_terms_title"} )
    {
        $title_text = $state->{"ana_terms_title"};
    }
    else
    {
        $root_node = $nodes->{ $state->{"ana_root_id"} };
    ...
    push @l_widgets, &Common::Widgets::title_box( $title_text, "ana_title" );

    # ------ Control menu,

    if ( $state->{"ana_control_menu_open"} ) {
        push @l_widgets, &Anatomy::Widgets::control_menu( $columns );
    } else {
        push @l_widgets, &Anatomy::Widgets::control_icon();
    }

    # ------ Data menu,

    if ( $state->{"ana_data_menu_open"} ) {
        push @l_widgets, &Anatomy::Widgets::data_menu( $columns, $db_prefix, $state );
    } else {
        push @l_widgets, &Anatomy::Widgets::data_icon();
    }
    
    # ------ Statistics menu,

    if ( $state->{"ana_statistics_menu_open"} ) {
        push @l_widgets, &Anatomy::Widgets::statistics_menu( $db_prefix, $columns );
    }

    # ------ Selections menu,

    if ( -r "$Common::Config::ses_dir/$sid/".$db_prefix."_selections" ) 
    {
        if ( $state->{"ana_selections_menu_open"} ) {
            push @l_widgets, &Anatomy::Widgets::selections_menu( $sid, $db_prefix, $state->{"ana_terms_title"} );
        }
    }
    
    # ------ Search icon,

    push @r_widgets, &Anatomy::Widgets::search_icon( $db_prefix, $sid );

    # ------ Help button,

    push @r_widgets, &Anatomy::Widgets::help_icon( $db_prefix, $sid );
...
sub main
{
    ...
    # The driving routine of the user interface. The routine first 
    ...
    my ( @xhtml, $state, $root_node, $child, $root_id, $dbh, $nodes,
         ...
         @ids, $request, $sel, $ids, $id, $menu, $options, $help_sub,
         $xhtml, $add_count, $display, $col, $index, $columns, $cols, 
         $sid, $infos, $info, $sels, @cols, $opt_index, $i, $type, 
         $menu_requests, $state_key, $state_val, $node, $db, $report_sub );
    ...
    # Set variables in other modules, so the right tables are used.
    # This is done in order to use the same code with different 
    # anatomy ontologies. 
    ...
    if ( $page eq "po" )
    {
        $db_prefix = "po";   
        $id_name = "po_id";
        $help_sub = \&Anatomy::Help::plant;
        $report_sub = \&Anatomy::Viewer::display_plant_report;
    }
    elsif ( $page eq "flyo" )
    {
        $db_prefix = "flyo";   
        $id_name = "flyo_id";
        $help_sub = \&Anatomy::Help::fly;
        $report_sub = \&Anatomy::Viewer::display_fly_report;
    }
    elsif ( $page eq "mao" )
    {
        $db_prefix = "mao";  
        $id_name = "mao_id";
        $help_sub = \&Anatomy::Help::mouse;
        $report_sub = \&Anatomy::Viewer::display_mouse_report;
    }
    elsif ( $page eq "mado" )
    {
        $db_prefix = "mado";  
        $id_name = "mado_id";
        $help_sub = \&Anatomy::Help::moused;
        $report_sub = \&Anatomy::Viewer::display_moused_report;
    }
    else
    {
        &error( qq (Unrecognized page -> "$page") );
        exit;
    }

    $Anatomy::DB::db_prefix = $db_prefix;
    $Anatomy::DB::id_name = $id_name;
    $Anatomy::DB::def_table = $db_prefix ."_def";
    $Anatomy::DB::edges_table = $db_prefix ."_edges";
    $Anatomy::DB::synonyms_table = $db_prefix ."_synonyms";
    $Anatomy::DB::xrefs_table = $db_prefix ."_xrefs";
    $Anatomy::DB::stats_table = $db_prefix ."_stats";

    $Anatomy::Schema::db_prefix = $db_prefix;
    $Anatomy::Schema::id_name = $id_name;
    $Anatomy::Schema::def_table = $db_prefix ."_def";
    $Anatomy::Schema::edges_table = $db_prefix ."_edges";
    $Anatomy::Schema::synonyms_table = $db_prefix ."_synonyms";
    $Anatomy::Schema::xrefs_table = $db_prefix ."_xrefs";
    $Anatomy::Schema::stats_table = $db_prefix ."_stats";
    ...
    $state = &Anatomy::Users::restore_state( $sid, $db_prefix );

    if ( not $state->{"ana_click_id"} ) {
        $state->{"ana_click_id"} = $state->{"ana_root_id"};
    }
    ...
    $display = &Anatomy::Viewer::restore_display( $dbh, $sid, $db_prefix, $state );
    ...
    if ( defined $state->{"ana_control_menu"} )
    {
        $opt_index = $state->{"ana_control_menu"};
        $state->{"request"} = &Anatomy::Menus::control_info( $db_prefix )->[ $opt_index ]->{"request"};

        $state->{"ana_control_menu"} = undef;
    }
    elsif ( defined $state->{"ana_data_menu"} )
    {
        $opt_index = $state->{"ana_data_menu"};
        $state->{"request"} = &Anatomy::Menus::data_info( $db_prefix )->[ $opt_index ]->{"request"};

        $state->{"ana_data_menu"} = undef;
    }
    ...
    if ( $request )
    {
        $state->{"ana_root_name"} = &Anatomy::DB::get_name_of_id( $dbh, $state->{"ana_root_id"} );
        $root_id = $state->{"ana_click_id"};
        ...
        $menu_requests->{"show_ana_control_menu"} = [ "ana_control_menu_open", 1 ];
        $menu_requests->{"hide_ana_control_menu"} = [ "ana_control_menu_open", 0 ];
        $menu_requests->{"show_ana_data_menu"} = [ "ana_data_menu_open", 1 ];
        $menu_requests->{"hide_ana_data_menu"} = [ "ana_data_menu_open", 0 ];
        $menu_requests->{"show_ana_statistics_menu"} = [ "ana_statistics_menu_open", 1 ];
        $menu_requests->{"hide_ana_statistics_menu"} = [ "ana_statistics_menu_open", 0 ];
        $menu_requests->{"show_ana_selections_menu"} = [ "ana_selections_menu_open", 1 ];
        $menu_requests->{"hide_ana_selections_menu"} = [ "ana_selections_menu_open", 0 ];
        $menu_requests->{"show_ana_expression_menu"} = [ "ana_expression_menu_open", 1 ];
        $menu_requests->{"hide_ana_expression_menu"} = [ "ana_expression_menu_open", 0 ];
        ...
        if ( $menu_requests->{ $request } )
        {
            ...
            &Anatomy::Viewer::set_row_checkbox_values( $display, $state->{"ana_row_ids"} );
            &Anatomy::Viewer::set_col_checkbox_values( $display, $state->{"ana_col_ids"}, 1 );
            
            &Anatomy::Viewer::save_display( $sid, $display );
        }
        ...
        {
            $xhtml = &Anatomy::Viewer::search_window( $session );

            $state->{"is_popup_page"} = 1;
            $state->{"title"} = "Anatomy Search Page";
            $state->{"viewer"} = "anatomy";
        ...
        {
            $xhtml = &Anatomy::Viewer::save_selection_window( $session );

            $state->{"is_popup_page"} = 1;
            $state->{"title"} = "Anatomy Save Selection Page";
            $state->{"viewer"} = "anatomy";

            &Common::Widgets::show_page( $xhtml, $session->param("session_dir"), $state );
            exit;
        }
        elsif ( $request eq "ana_report" )
        {
            $xhtml = &{ $report_sub }( $session, $state->{"ana_report_id"} );

            $state->{"is_popup_page"} = 1;
            $state->{"title"} = "Anatomy Report Page";
            $state->{"viewer"} = "anatomy";
        ...
        # Anatomy::Menus::control_info
        
        elsif ( $request eq "add_unchecked_column" ) 
        {
            $display = &Anatomy::Viewer::add_checkbox_column( $dbh, $display );

            &Anatomy::Viewer::set_col_checkbox_values( $display, $state->{"ana_col_ids"}, 1 );
            &Anatomy::Viewer::save_display( $sid, $display );

            $state->{"ana_save_rows_button"} = 1;
        }
        elsif ( $request eq "add_checked_column" ) 
        {
            $display = &Anatomy::Viewer::add_checkbox_column( $dbh, $display, "checked" );
        ...
        {
            $display = &Anatomy::Viewer::add_checkbox_row( $dbh, $display, $request );

            &Anatomy::Viewer::set_row_checkbox_values( $display, $state->{"ana_row_ids"} );
            &Anatomy::Viewer::save_display( $sid, $display );

            $state->{"ana_delete_cols_button"} = 1;
            $state->{"ana_cols_checkboxes"} = 1;
        }
        elsif ( $request eq "add_checked_row" ) 
        {
            $display = &Anatomy::Viewer::add_checkbox_row( $dbh, $display, $request, "checked" );
        ...
        elsif ( $request eq "add_compare_row" ) 
        {
            ...
            $state->{"ana_compare_cols_button"} = 1;
            $state->{"ana_cols_checkboxes"} = 1;
        }

        # >>>>>>>>>>>>>>>>>>>>>> DELETE CHECKBOX ROW <<<<<<<<<<<<<<<<<<<<<<<<<<

        elsif ( $request eq "hide_ana_col_checkboxes" )
        {
            ...
            $state->{"ana_delete_cols_button"} = 0;
            $state->{"ana_cols_checkboxes"} = 0;
            $state->{"ana_compare_cols_button"} = 0;
        }
        ...
        {
            $display = &Anatomy::Viewer::add_data_column( $dbh, $display, $opt_index, undef, $root_id );
            $state->{"ana_data_menu"} = undef;
        ...
        {
            $opt_index = $state->{"ana_statistics_menu"};
            $display = &Anatomy::Viewer::add_statistics_column( $dbh, $display, $opt_index, undef, $root_id );

            $state->{"ana_statistics_menu"} = undef;
        ...
        elsif ( $request eq "delete_column" )
        {
            ...
            if ( $display->{"columns"}->[ $state->{"ana_col_index"} ]->{"type"} eq "checkbox" ) {
                $state->{"ana_save_rows_button"} = 0;
            }

            &Anatomy::Viewer::delete_columns( $display, $state->{"ana_col_index"} );
        ...
        {
            &Anatomy::Viewer::delete_columns( $display, $state->{"ana_col_ids"} );
        ...
        {
            $info = &Common::Users::ana_selection_new( $dbh, $state );

            $info->{"ids"} = $state->{"ana_row_ids"};
            $state->{"screen_message"} = &Anatomy::Viewer::save_selection( $sid, $info );
        ...
        {
            $index = $state->{"ana_info_index"};
            $sels = &Common::Users::ana_selections_delete( $sid, $index );
            ...
            if ( $sels )
            {
                ...
                $display = &Anatomy::Viewer::rebuild_display( $dbh, $sid, $state, $display, $sel );

                &Anatomy::Viewer::set_row_checkbox_values( $display, $sel->{"ids"} );
                &Anatomy::Viewer::set_col_checkbox_values( $display, $state->{"ana_col_ids"}, 1 );
                
                $root_node = &Common::DAG::Nodes::get_nodes_ancestor( $display->{"nodes"}, $sel->{"ids"} );
                $state->{"ana_root_id"} = &Common::DAG::Nodes::get_id( $root_node );

                $state->{"ana_delete_rows_button"} = 1;
                $state->{"ana_selections_menu"} = $index;
            }
            else
            {
                $state->{"request"} = "focus_node";
                $state->{"ana_click_id"} = 1;
                $state->{"ana_terms_title"} = $state->{"root_name"};

                $display = &Anatomy::Viewer::update_display( $dbh, $sid, $display, $state );

                $state->{"ana_delete_rows_button"} = 0;
                $state->{"ana_selections_menu_open"} = 0;
            }
        ...
        {
            $index = $state->{"ana_selections_menu"};
            $sel = &Anatomy::Menus::selections_info( $sid, $db_prefix )->[ $index ];
        ...
        {
            $state->{"ana_root_id"} = $state->{"ana_search_id"};
    
            $display->{"nodes"} = &Anatomy::Viewer::text_search( $dbh, $state );
            $display->{"columns"} = [];

            $display = &Anatomy::Viewer::create_columns( $dbh, $sid, $display, $columns, $state->{"ana_root_id"} );

            &Anatomy::Viewer::save_display( $sid, $display );

            $root_node = $display->{"nodes"}->{ $state->{"ana_root_id"} };
            $state->{"ana_root_name"} = $root_node->{"name"};
            
            $state->{"ana_click_id"} = "";
            $state->{"ana_terms_title"} = "";
        }
...
#    &dump( scalar keys %{ $display->{"nodes"} } );

    # Add column styles,

    $display = &Anatomy::Viewer::add_col_styles( $state, $display );
    ...
    @xhtml = &Anatomy::Viewer::display_menu_row( $sid, $display, $state );
...
<input type="hidden" name="ana_click_id" value="" />
<input type="hidden" name="ana_info_type" value="" />
<input type="hidden" name="ana_info_key" value="" />
<input type="hidden" name="ana_info_tip" value="" />
<input type="hidden" name="ana_info_col" value="" />
<input type="hidden" name="ana_info_menu" value="" />
<input type="hidden" name="ana_info_index" value="" />
<input type="hidden" name="ana_col_index" value="" />
<input type="hidden" name="ana_info_ids" value="" />
<input type="hidden" name="ana_col_ids" value="" />
<input type="hidden" name="ana_row_ids" value="" />
<input type="hidden" name="ana_show_widget" value="" />
<input type="hidden" name="ana_hide_widget" value="" />
);

    # -------- Save button and delete button

    push @xhtml, &Anatomy::Viewer::display_action_buttons( $sid, $state, $display );
    ...
    push @xhtml, &Anatomy::Viewer::display_checkbox_row( $display );

    # ------ Optional row with column headers and parents menu,

    push @xhtml, &Anatomy::Viewer::display_col_headers( $dbh, $display, $state );

    # ------ Display children only if at the root,

    $root_id = $state->{"ana_root_id"};

    push @xhtml, &Anatomy::Viewer::display_rows( $sid, $display, $root_id, undef, 0, $state );
    
    push @xhtml, qq (</table>\n);
    push @xhtml, qq (<table><tr><td height="15">&nbsp;</td></tr></table>\n);

    &Anatomy::Users::save_state( $sid, $db_prefix, $state );
    
    return join "", @xhtml;
}

sub display_fly_report
{
    # Niels Larsen, October 2004.
    ...
    $entry = &Anatomy::DB::get_fly_entry( $dbh, $id );

    &Common::DB::disconnect( $dbh );
    
    $xhtml = &Anatomy::Widgets::report_title( "Anatomy Ontology Entry" );
...
<tr><td colspan="3"><table class="ana_report_title"><tr><td>Description</td></tr></table></td></tr>
<tr><td width="$width"></td><td class="ana_report_key">ID</td><td class="ana_report_value">$id</td><td width="$width"></td></tr>
<tr><td width="$width"></td><td class="ana_report_key">Name</td><td class="ana_report_value">$text</td><td width="$width"></td></tr>
);
    ...
    if ( @{ $entry->{"synonyms"} } )
    {
        foreach $elem ( @{ $entry->{"synonyms"} } )
        {
            ...
            $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Synonym</td>)
                    . qq (<td class="ana_report_value">$text</td><td width="$width"></td></tr>\n);
        }
    }
    else {
        $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Synonyms</td>)
                . qq (<td class="ana_report_value">&nbsp;</td><td width="$width"></td></tr>\n);
    }
    ...
    $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Description</td>)
            . qq (<td class="ana_report_value">$text</td><td width="$width"></td></tr>);
    ...
    $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Comment</td>)
            . qq (<td class="ana_report_value">$text</td><td width="$width"></td></tr>);
    ...
    $xhtml .= qq (
<tr height="8"><td></td></tr>
<tr><td colspan="3"><table class="ana_report_title"><tr><td>Cross references</td></tr></table></td></tr>
);
    if ( @{ $entry->{"references"} } )
    {
        foreach $elem ( sort { $a->{"db_name"} cmp $b->{"db_name"} } @{ $entry->{"references"} } )
        {
            $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">$elem->{"db_name"}</td>)
                    . qq (<td class="ana_report_value">$elem->{"db_id"}</td><td width="$width"></td></tr>\n);
        }
    }
    else 
    {
        $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">References</td>)
                . qq (<td class="ana_report_value">&nbsp;</td><td width="$width"></td></tr>\n);
    }        
    ...
    $xhtml .= qq (
<tr height="8"><td></td></tr>
<tr><td colspan="3"><table class="ana_report_title"><tr><td>Parents and Children</td></tr></table></td></tr>
);
    ...
    if ( @{ $entry->{"parents"} } )
    {
        ...
        foreach $elem ( sort { $a->{"terms_total"} <=> $b->{"terms_total"} } @{ $entry->{"parents"} } )
        {
            ...
            $text .= qq ( (ID $elem->{ $id_name }));
            
            push @options, [ $elem->{ $id_name }, $text ];
        }
    
        if ( @options )
        {
            $menu = &Common::Widgets::select_menu( "ana_parents_menu",      # Element name
                                                   \@options,              # List of tuples
                                                   $options[0]->[1],       # Selected option
                                                   "beige_menu",           # Style class 
                                                   );
            
            $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Parents</td>)
                   . qq (<td class="ana_report_value" style="padding-top: 0px; padding-bottom: 0px;)
    ...
    if ( @{ $entry->{"children"} } )
    {
        ...
        if ( @options )
        {
            ...
            $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">Children</td>)
                   . qq (<td class="ana_report_value" style="padding-top: 0px; padding-bottom: 0px;)
                   . qq (padding-left: 1px;">$menu</td><td width="$width"></td></tr>\n);
        }
    }
  ...
  <div class="ana_report_author">Data maintained by the <strong>Flybase Consortium</strong>.</div>
  </td></tr></table>\n);

    return $xhtml;
}

sub display_mouse_report
{
    # Niels Larsen, October 2004.
    ...
    $entry = &Anatomy::DB::get_plant_entry( $dbh, $id );
...
<tr><td colspan="3"><table class="ana_report_title"><tr><td>Cross references</td></tr></table></td></tr>
    ...
    {
        $xhtml .= qq (<tr><td width="$width"></td><td class="ana_report_key">ICD9</td>)
                . qq (<td class="ana_report_value">&nbsp;</td><td width="$width"></td></tr>\n);
    }        
...
<tr><td colspan="3"><table class="ana_report_title"><tr><td>Parents and Children</td></tr></table></td></tr>
    ...
    $xhtml .= "</table>\n";
     ...
     $xhtml .= qq (<table width="100%"><tr height="60"><td align="right" valign="bottom">
  <div class="ana_report_author">Data maintained by the <strong>Plant Ontology Consortium</strong>.</div>
  </td></tr></table>\n);

    return $xhtml;
}

sub display_plant_report
{
    # Niels Larsen, October 2004.
...
sub display_rows
{
    ...
    push @xhtml, &Anatomy::Viewer::display_row( $sid, $columns, $display->{"nodes"}->{ $nid },
                                                $nrel, $indent, $state );
    $indent += 1;
                     ...
                     &Common::DAG::Nodes::get_children_list( $display->{"nodes"}, $nid ) )
    {
        ...
        push @xhtml, &Anatomy::Viewer::display_rows( $sid, $display, $cid, $rel, $indent, $state );
    }
...
sub display_row
{
    ...
    for ( $col_ndx = 0; $col_ndx <= $#{ $columns }; $col_ndx++ )
    {
        ...
        if ( $type eq "statistics" )
        {
            ...
            if ( $key eq $db_prefix."_terms_tsum" or $key eq $db_prefix."_terms_usum" )
            {
                $xhtml .= &Anatomy::Viewer::display_stats_cell( $node, $key, $col_ndx, 1, 1, 1 );
            }
        }
        elsif ( $type eq "data" )
        {
            ...
            if ( $key eq $id_name )
            {
                $xhtml .= &Anatomy::Viewer::display_id_cell( $sid, $node, $nrel, $col_ndx );
            }
        }
        ...
        {
            $xhtml .= &Anatomy::Viewer::display_checkbox_cell( $node, $col_ndx );
        }
        else
        {
...
sub display_stats_cell
{
    ...
    # Displays the content of a "cell" of the table of anatomy groups
    ...
    if ( defined $value_sum )
    {
        ...
        {
            $link = qq (<img src="$Common::Config::img_url/report.gif" alt="Anatomy" />);
            $links = "";
...
sub parents_menu
{
    ...
    $title ||= "ana_parents_menu";

    my ( $root_node, $name, $p_nodes, $id, $node, $xhtml, @options, 
         @menu_ids, $stats, $root_id );

    $root_node = $nodes->{ $state->{"ana_root_id"} };
    $root_id = $root_node->{ $id_name };

    $name = &Common::Names::format_display_name( $root_node->{"name"} );

    $p_nodes = &Anatomy::DB::get_parents( $dbh, $root_id );
    delete $p_nodes->{ 0 };  # TODO: eliminate this

    @menu_ids = &Common::DAG::Nodes::get_ids_all( $p_nodes );

    $stats = &Anatomy::DB::get_statistics( $dbh, $db_prefix."_terms_tsum", [ @menu_ids, $root_id ] );

    @options = [ $root_node->{ $id_name }, $name, $stats->{ $root_id }->{ $db_prefix."_terms_tsum"} ];
    ...
    foreach $id ( @menu_ids )
    {
        ...
        push @options, [ $id, $name, $stats->{ $id }->{ $db_prefix."_terms_tsum"} ];
    }
    ...
    $xhtml = &Common::Widgets::select_menu( $title,                 # Element name
                                            ...
                                            "javascript:handle_parents_menu(this.form.ana_parents_menu)" );
...
sub rebuild_display
{
    ...
    $nodes = &Anatomy::DB::get_nodes( $dbh, $sel->{"ids"} );
    $p_nodes = &Anatomy::DB::get_nodes_parents( $dbh, $nodes );
    ...
    $display->{"columns"} = [];
    $display = &Anatomy::Viewer::create_columns( $dbh, $sid, $display, $columns );

    # Update state,

    $state->{"ana_terms_title"} = $sel->{"menu"};
...
sub restore_display
{
    ...
    # Restores the display tree as a nodes hash fron the file ana_display
    ...
    $file = "$Common::Config::ses_dir/$sid/".$db."_display";
...
#        $display = &Common::File::eval_file( $file );
    ...
    else
    {
        ...
        $info = &Anatomy::Menus::statistics_info( $db );
        $cols = [ $info->[1] ];

        $id = $state->{"ana_root_id"};
        $depth = 1;

        $nodes = &Anatomy::DB::open_node( $dbh, $id, $depth );

        $parent_nodes = &Anatomy::DB::get_parents( $dbh, $id );
        $nodes = &Common::DAG::Nodes::merge_nodes( $nodes, $parent_nodes, 1 );

        $nodes = &Common::DAG::Nodes::delete_ids_parent_orphan( $nodes, $id );
        $nodes = &Common::DAG::Nodes::set_ids_children_all( $nodes );

        $display->{"nodes"} = $nodes;

        $display = &Anatomy::Viewer::create_columns( $dbh, $sid, $display, $cols, $id );
...
#        &Common::File::dump_file( $file, $display );
        ...
        $list = &Anatomy::Menus::selections_info( $sid, $db_prefix );
        &Anatomy::Users::save_selections( $sid, $db_prefix, $list );
    }
...
sub save_display
{
    ...
    # Saves the display to the file ana_display under the user's directory. 
    ...
    $file = "$Common::Config::ses_dir/$sid/".$db_prefix."_display";
...
sub save_selection 
{
    ...
    {
        $list = &Common::Users::ana_selections_add( $sid, $info, $index );
...
sub save_selection_window
{
    ...
    my ( $state, $title, $title_bar, $xhtml, $sid, $name );
...
<input type="hidden" name="ana_info_key" value="$name" />

<div id="search_box">
<table cellspacing="5">
<tr height="25">
   <td align="right">Menu title&nbsp;&raquo;</td>
   <td colspan="3"><input type="text" size="25" maxlength="25" name="ana_search_text" value="">
</tr>
<tr height="25">
   <td align="right">Column header&nbsp;&raquo;</td>
   <td colspan="3"><input type="text" size="4" maxlength="4" name="ana_search_text" value="" /></td>
</tr>
<tr>
   <td></td>
...
sub search_window
{
    ...
    $name = &Anatomy::DB::get_name_of_id( $dbh, $state->{"ana_root_id"} );
    ...
    $search_text = $state->{"ana_search_text"} || "";
    ...
    $nodes = &Anatomy::DB::get_nodes( $dbh, [ $state->{"ana_root_id"} ] );
    $p_nodes = &Anatomy::DB::get_parents( $dbh, $state->{"ana_root_id"} );

    $nodes = &Common::DAG::Nodes::merge_nodes( $nodes, $p_nodes );

    $search_titles_menu = &Anatomy::Viewer::parents_menu( $dbh, $nodes, $state, "ana_search_id" );

    &Common::DB::disconnect( $dbh );

    # -------- Target select menu,

    $search_target = $state->{"ana_search_target"};
    ...
    $search_target_menu = &Common::Widgets::select_menu( "ana_search_target",
                                                         \@target_options,
                                                         $search_target,
                                                         "grey_menu" );
    # -------- Type select menu,

    $search_type = $state->{"ana_search_type"};
    ...
    $search_type_menu = &Common::Widgets::select_menu( "ana_search_type",
                                                       \@type_options,
                                                       $search_type,
                                                       "grey_menu" );
...
<tr height="25">
   ...
   <td align="left"><input type="text" size="50" name="ana_search_text" value=" $search_text" /></td>
</tr>
<tr height="25">
...
sub text_search
{
    ...
    $root_id = $state->{"ana_root_id"};
    $text = $state->{"ana_search_text"};
    $type = $state->{"ana_search_type"};
    $target = $state->{"ana_search_target"};
    ...
    $nodes = &Anatomy::DB::open_node( $dbh, $root_id, 1 );

    # Get matching ids, if any, 

    @ids = &Anatomy::DB::text_search( $dbh, $root_id, $text, $type, $target );
    
    if ( @ids )
    {
        @ids = &Common::Util::uniqify( \@ids );

        $match_nodes = &Anatomy::DB::get_nodes( $dbh, \@ids, 0, 1 );
    ...
    $p_nodes = &Anatomy::DB::get_nodes_parents( $dbh, $nodes, 0, 1 );
    $nodes = &Common::DAG::Nodes::merge_nodes( $nodes, $p_nodes );
...
sub update_display
{
    ...
    my ( $request, $root_id, $id, $node, $tables, $depth, $col, $col_id,
    ...
    if ( not defined $state ) {
        $state = &Anatomy::Users::default_state( $db_prefix );
    }
    ...
    $root_id = $state->{"ana_root_id"};
    $click_id = $state->{"ana_click_id"};

    if ( $state->{"ana_root_id"} ) {
        $depth = $nodes->{ $state->{"ana_root_id"} }->{"depth"};
    } else {
    ...
    if ( $request eq "close_node" or $request eq "collapse_node" )
    {
        ...
        $nodes = &Common::DAG::Nodes::delete_ids_parent_orphan( $nodes );
        $nodes = &Common::DAG::Nodes::delete_ids_child_orphan( $nodes );
    ...
    else
    {
        ...
        {
            $click_id = &Anatomy::DB::get_id_of_name( $dbh, $state->{"ana_root_name"} );

            $state->{"ana_root_id"} = $click_id;
            $state->{"ana_click_id"} = $click_id;
        ...
        if ( $request eq "open_node" )
        {
            ...
            $new_nodes = &Anatomy::DB::open_node( $dbh, $click_id, $depth );
            ...
            $display = &Anatomy::Viewer::create_columns( $dbh, $sid, $display, $columns, $click_id );
        }
        elsif ( $request eq "focus_node" )
        {
            # Fetches new nodes from the database where the clicked id is the parent,

            $nodes = &Anatomy::DB::open_node( $dbh, $click_id, $depth, 3 );

            $parent_nodes = &Anatomy::DB::get_parents( $dbh, $click_id );
            $nodes = &Common::DAG::Nodes::merge_nodes( $nodes, $parent_nodes, 1 );
        ...
        elsif ( $request eq "expand_node" )
        {
            ...
            $col_id = $state->{"ana_col_index"};
            $col = $columns->[ $col_id ];

            $new_nodes = &Anatomy::DB::expand_node( $dbh, $click_id, 9999 );
