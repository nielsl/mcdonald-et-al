...
# Accessor routines for the disease viewer. This library in turn uses
...
# than one disease ontology (human, mouse, plant, .. ) for which we 
...
sub get_node
{
    ...
    $node = &Disease::DB::get_nodes( $dbh, [ $id ], $select, $def_table, 
                                     $edges_table )->{ $id };
...
sub text_search
{
    ...
    if ( $search_type eq "whole_words" )
    {
        ...
        $match_ref = qq ( match (do_xrefs.name) against ('$search_text'));
    }
