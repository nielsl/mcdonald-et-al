...
sub accs_to_gis
{
    ...
    @gis = &Common::Entrez::_efetch_ids( $accs, {
...
sub accs_to_seq_summaries
{
    ...
    return &Common::Entrez::accs_to_summaries(
        $accs,
        {
            "db" => "sequences",
            "dbtype" => $type,
...
sub taxids_to_summaries
{
    ...
    return &Common::Entrez::fetch_summaries( $ids, {
            "db" => "taxonomy",
            "dbtype" => "orgs_tax",
            "idtype" => "tax",
            "oformat" => "stream",
...
sub accs_to_summaries
{
    ...
    @stones = &Common::Entrez::fetch_summaries( $accs, {
        "db" => $args->db,
        "dbtype" => $args->dbtype,
...
sub _efetch_ids
{
    ...
    @uniq_ids = &Common::Entrez::uniqify( $ids );
    @uniq_ids = &Common::Entrez::_edit_id_underscores( \@uniq_ids );
    ...
    for ( $ndx = 0; $ndx <= $#uniq_ids; $ndx += $ret_max )
    {
        ...
        while ( $try <= $try_max )
        {
            $epoch = &Common::Entrez::_wait_a_moment_if_needed( $epoch );
...
sub _esummary
{
    ...
    my ( $db, $try_max, $try, @ids, $url, $ua, $ndx, $req, $res, $xml, @xml,
         $email, %out_ids, $i, $j, @summaries, $format, $epoch, @errors, $idstr,
         $wait_max );

    $args = &Registry::Args::check(
        $args, {
            "S:1" => [ qw ( db trymax waitmax oformat ) ],
        });

    # >>>>>>>>>>>>>>>>>>>>>>>>> FETCH FROM NCBI <<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    @ids = &Common::Entrez::uniqify( $ids, $msgs );
    ...
    while ( $try <= $try_max )
    {
        ...
        if ( $res->is_success )
        {
            ...
            if ( @errors = ( $xml =~ /<ERROR>([^<]+)<\/ERROR>/isg ) or 
                 @errors = ( $xml =~ /Error: ([^\n]+)/isg ) )
            {
    ...
    {
        &Common::Messages::error( qq (
No summaries returned from NCBI, tried $try_max times, each waiting for $wait_max 
...
between you and them. We recommend re-submitting some time later. 
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>> REPEAT IF OUTPUT TRUNCATED <<<<<<<<<<<<<<<<<<<<<<<<<<

    # Problem: sometimes the xml output is truncated .. so here the xml string is 
    # checked for completion, repaired so it can be parsed, and then below we see 
    # which ids are missing if any,

    if ( $xml !~ /<\/eSummaryResult>\s+$/s )
    {
        @xml = split "\n\n", $xml;
        $xml = join "\n\n", @xml[ 0 .. $#xml-1 ];
    ...
    if ( ($format = $args->oformat) eq "stream" )
    {
        push @summaries, &Common::Entrez::xml_to_stream( \$xml );
    }
    else {
        &Common::Messages::error( qq (Wrong looking format -> "$format"; format must be "stream") );
    }
    ...
    if ( $i > $j )
    {
        %out_ids = map { $_, 1 } map { $_->Id } @summaries;
        @ids = grep { not exists $out_ids{ $_ } } @ids;
            
        if ( $args->trymax > 1 )
        {
            push @summaries, &Common::Entrez::_esummary( \@ids, {
                "db" => $args->db,
                "trymax" => $args->trymax - 1,
...
sub fetch_seqs_file
{
    ...
    $output = &Common::Entrez::_efetch_records( $ids, {
        "db" => &Common::Entrez::set_db( $args->dbtype ),
        "rettype" => &Common::Entrez::set_rettype( $args->dbtype ),
        "retmode" => "text",
        "retmax" => 50,
        "trymax" => 3,
        "waitmax" => 30,
...
sub fetch_subseq
{
    ...
    $output = &Common::Entrez::_efetch_records( [ $id ], $conf, $msgs );
...
sub fetch_summaries
{
    ...
    my ( $type, @output, %gi_to_id, @uniq_ids, %uniq_ids, @ids, $format, 
         %valid, $xml, $ndx, $i, $j, $obj, @gis, @list, $ret_max, $epoch, 
        );
    
    $args = &Registry::Args::check(
        $args, {
            "S:2" => [ qw ( idtype db dbtype oformat ) ],
            "S:0" => [ qw ( debug rettype retmode retmax trymax waitmax ) ],
        });
    
    $args->rettype("gi") if not defined $args->rettype;
    $args->retmode("asn.1") if not defined $args->retmode; # The only format available - bug submitted 
    $args->rettype("gi") if not defined $args->rettype;
    $args->retmax( 200 ) if not $args->retmax;
    $args->trymax( 5 ) if not $args->trymax;
    $args->waitmax( 60 ) if not $args->waitmax;
    ...
    if ( $valid{ $args->idtype } )
    {
        ...
        {
            @gis = &Common::Entrez::accs_to_gis( \@uniq_ids,
                                                 {
                                                     "db" => $args->db,
                                                     "rettype" => $args->rettype,
                                                     "retmode" => $args->retmode, 
    ...
    for ( $ndx = 0; $ndx <= $#uniq_ids; $ndx += $ret_max )
    {
        ...
        @list = &Common::Entrez::_esummary( \@ids, {
            "db" => &Common::Entrez::set_db( $args->dbtype ),
            "trymax" => 3,
            "waitmax" => 90,
            "oformat" => $args->oformat,
...
sub gis_to_accs
{
    ...
    @accs = &Common::Entrez::_efetch_ids( $gis, {
        "db" => "sequences",
        "rettype" => "acc",
        "retmode" => "text", 
        "retmax" => 500,
...
sub _wait_a_moment_if_needed
{
    my ( $epoch,
        ) = @_;

    # Wait until at least 3 seconds have passed since last request,

    my ( $cur_epoch );

    $epoch = 0 if not defined $epoch;
    $cur_epoch = &Common::Util::time_string_to_epoch();
    
    if ( $cur_epoch - $epoch < 5 )
    {
        &dump( "waiting ". ($epoch - $cur_epoch + 4) . " seconds" );
        sleep $epoch - $cur_epoch + 4;
    }
...
Do not delete, belongs to _efetch_ids
