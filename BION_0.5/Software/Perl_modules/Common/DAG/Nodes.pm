...
sub is_leaf
{
    ...
    # Tests if a given node is a terminal node (leaf) or not. 
...
sub get_ids_parents_all
{
    ...
    $parents = &Common::DAG::Nodes::get_parents_all( $nodes, $nid );
...
sub get_ids_subtree
{
    ...
    @ids = &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } );

    foreach $id ( @ids )
    {
        push @ids, &Common::DAG::Nodes::get_ids_children( $nodes->{ $id } );
    }
...
sub get_ids_subtree_total
{
    ...
    foreach $id ( @ids )
    {
        $count = &Common::DAG::Nodes::get_ids_subtree_total( $nodes, $id, $count );
    }
...
sub get_ids_subtree_unique
{
    ...
    foreach $id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } ) )
    {
        if ( not exists $nids->{ $id } )
        {
            $nids->{ $id } = 1;
            $nids = &Common::DAG::Nodes::get_ids_subtree_unique( $nodes, $id, $nids );
        }
    }
...
sub get_nodes_ancestor
{
    ...
    foreach $id ( @{ $ids } )
    {
        $parents = &Common::DAG::Nodes::get_parents_all( $nodes, $nodes->{ $id } );
    ...
    $p_id = &Common::DAG::Nodes::get_id_root( $nodes );
...
sub get_parents_all
{
    ...
    $p_nodes = &Common::DAG::Nodes::get_parents( $nodes, $node );
    ...
    foreach $p_id ( keys %{ $p_nodes } )
    {
        if ( not exists $parents->{ $p_id } )
        {
            ...
            $p_nodes2 = &Common::DAG::Nodes::get_parents_all( $nodes, $nodes->{ $p_id }, $parents );
...
sub get_subtree 
{
    ...
    %children = &Common::DAG::Nodes::get_children( $nodes, $nid );

    foreach $child_id ( keys %children )
    {
        %children2 = &Common::DAG::Nodes::get_subtree( $nodes, $child_id, 0 );
...
sub set_node
{
    ...
    $nodes->{ $node->{ $id_name } } = &Storable::dclone( $node );
...
sub set_name
{
    ...
    return &Common::DAG::Nodes::set_label( $node, "name", $value );
}
...
sub set_type
{
    ...
    return &Common::DAG::Nodes::set_label( $node, "type", $value );
}
...
sub set_ids_children_subtree
{
    ...
    $sub_ids = &Common::DAG::Nodes::get_ids_subtree_unique( $nodes, $nid );
...
sub set_label_subtree 
{
    ...
    foreach $child_id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid  } ) )
    {
        &Common::DAG::Nodes::set_label_all( $nodes, $key, $child_id );
    }
...
sub set_depth_all
{
    ...
    if ( not defined $nid ) {
        $nid = &Common::DAG::Nodes::get_id_root( $nodes );
    }

    $depth = 0 if not defined $depth;

    &Common::DAG::Nodes::set_label( $nodes->{ $nid }, "depth", $depth );

    foreach $child_id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } ) )
    {
        &Common::DAG::Nodes::set_depth_tree( $nodes, $child_id, $depth+1 );
    }
...
sub delete_ids_parent_orphan
{
    ...
    # If a root node id is given, the routine also removes parent ids
    # to visible nodes other than the root node. This prevents nodes
    # with multiple parents from being shown multiple times unless they
    ...
    my ( $id, $p_id, $node, $i, @p_ids, @p_rels, $hash );

    if ( defined $root_id )
    {
        $hash = { map { $_, 1 } &Common::DAG::Nodes::get_ids_parents_all( $nodes, $root_id ) };
        $hash->{ $root_id } = 1;
        ...
        foreach $id ( keys %{ $nodes } )
        {
            ...
            {
                if ( exists $nodes->{ $p_id } and exists $hash->{ $p_id } )
                {
                    push @p_ids, $p_id;
...
sub delete_label_subtree
{
    ...
    foreach $child_id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } ) )
    {
        &Common::DAG::Nodes::delete_label_all( $nodes, $child_id, $key );
    }
...
sub delete_subtree
{
    ...
    # The subtree originating at a given node is deleted. If nodes within
    # the deleted subtree are children of visible nodes outside the subtree,
    # then they are kept, but their references to parents within the subtree 
    # are deleted. If the third argument is true the top node of the subtree
    # is kept (but children ids deleted), otherwise it is deleted. 

    my ( $nodes,   # Nodes hash
         $rid,     # Root ID 
         $nid,     # Subtree top node ID
         $keep,    # Whether to keep starting node - OPTIONAL (default 1)
         ) = @_;

    # Returns a modified nodes hash.
    
    my ( $s_ids, $keep_ids, $p_ids, $p_id, $id, $node );

    $keep = 1 if not defined $keep;

    $p_ids = { map { $_, 1 } &Common::DAG::Nodes::get_ids_parents_all( $nodes, $rid ) };
    $p_ids->{ $rid } = 1;

    $s_ids = { map { $_, 1 } &Common::DAG::Nodes::get_ids_subtree( $nodes, $nid ) };

    foreach $id ( keys %{ $nodes } ) 
    {
        if ( not exists $p_ids->{ $id } and not exists $s_ids->{ $id } ) {
            $keep_ids->{ $id } = 1;
        }
    }

    foreach $id ( keys %{ $s_ids } )
    {
        if ( $id == $nid )
        {
            if ( $keep )
        ...
        {
            $node = $nodes->{ $id };
            @{ $node->{"parent_ids"} } = grep { $keep_ids->{ $_ } } @{ $node->{"parent_ids"} };
            
            if ( not @{ $node->{"parent_ids"} } ) {
                &Common::DAG::Nodes::delete_node( $nodes, $id, 0 );
            }
        }
    }
...
sub merge_node
{
    ...
    $new_node = &Common::DAG::Nodes::copy_node( $node1 );
...
sub traverse_head
{
    ...
    foreach $child_id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } ) )
    {
        &Common::DAG::Nodes::tree_traverse_head( $nodes, $child_id, $subref, $argref );
    }
...
sub traverse_tail
{
    ...
    foreach $child_id ( &Common::DAG::Nodes::get_ids_children( $nodes->{ $nid } ) )
    {
        &Common::DAG::Nodes::traverse_tail( $nodes, $child_id, $subref, $argref );
    }
...
sub delete_ids_outside_set
{
    ...
    foreach $id ( @{ $setids } )
    {
        foreach $p_id ( @{ $nodes->{ $id }->{"parent_ids"} } )
        {
            ...
            {
                $nodes = &Common::DAG::Nodes::delete_id_parent( $nodes, $id, $p_id );
            }
        }
    }
