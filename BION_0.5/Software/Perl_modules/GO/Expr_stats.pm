...
sub schema
{
    ...
    # Schema table for function column statistics. 

    # Returns an array.

    my $schema = [
                  [ "go_id", "int unsigned not null", "index go_id_ndx (go_id)" ],
                  [ "exp_id", "int unsigned not null default 0" ],
...
sub update_go
{
    ...
    $schema = &GO::Stats::schema;
    $id_ndx = &GO::Stats::schema_column_index( $schema, "go_id" );

    foreach $col_name ( "go_terms_usum", "go_terms_tsum" )
    {
        $col_ndx = &GO::Stats::schema_column_index( $schema, $col_name );
...
sub update_go_orgs
{
    ...
    $schema = &GO::Stats::schema;
    $id_ndx = &GO::Stats::schema_column_index( $schema, "id" );
